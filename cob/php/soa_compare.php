    ...
    <div id="main">
        ...
        <div style="margin: auto;max-width: 1200px;">
            <div style="font-family: 'Raleway', sans-serif;margin-left:15px;margin-right:10px;margin-bottom:15mm;">
                <h3 style="margin-top:30px;margin-bottom:10px;">Precision-Recall Frameworks - PASCAL Context test set</h3>
                ...
                <i>Dashed lines refer to methods which do not provide segmentations, but contour maps. For comparison on BSDS500, visit the <a href="http://www.vision.ee.ethz.ch/~cvlsegmentation/seism/soa_compare.php" target="_blank">SEISM website</a>.</i>

                <h3 style="margin-top:30px;margin-bottom:1px;">Legend</h3>
                <ul id="legend_text">
                </ul>
                Is your technique missing although it's published and the code is public? <a href="contact.php">Let us know</a> and we'll add it.
            </div>
        </div>
    </div>
</div>
<script>
    ...
    // ***************************************************************************************************************
    var techniques = ["COB_trainval","CEDN","HED_trainval","LEP-BSDS500", "MCG-BSDS500", "SE-BSDS500"];

    // Plot styles
    techn_plot_styles =
                {"LEP-BSDS500":{"legend_name": "LEP-BSDS500",
                        "color": "orange",
                        "type" : "line",
                        "style": "solid"},
                  "MCG-BSDS500":{"legend_name": "MCG-BSDS500",
                            "color": "red",
                            "type" : "line",
                            "style": "solid"},
                  "SE-BSDS500":{"legend_name": "SE-BSDS500",
                            "color": "black",
                            "type" : "line",
                            "style": "dashed"},
                  "CEDN":{"legend_name": "CEDN",
                            "color": "gray",
                            "type" : "line",
                            "style": "dashed"},
                  "HED_trainval":{"legend_name": "HED",
                            "color": "cyan",
                            "type" : "line",
                            "style": "dashed"},
                  "COB_trainval": {"legend_name": "COB",
                            "color": "black",
                            "type" : "line",
                            "style": "solid"}, 
    ...
    function do_one_plot(div_id, measure_name, database, gt_set)
    {
        ...
        {
          if (techn_plot_styles[techniques[ii]].style=="solid")
            plot_one(svg, techniques[ii], database, gt_set);
          else if (measure_name=="fb")
            plot_one(svg, techniques[ii], database, gt_set);
        }
        ...
        var n_tec = 0;
        for (var ii = 0, glob_ii = 0; ii < techniques.length; ii++)
        { 
          if (techn_plot_styles[techniques[ii]].style=="dashed" && measure_name=="fop")
            continue;
          n_tec++;
        }

        var ncols = Math.ceil(n_tec/leg_styles[measure_name].nrows);
        var nrows;
        if (n_tec>leg_styles[measure_name].nrows)
        ...
        { 
            ...
            var curr_col = Math.floor(glob_ii/leg_styles[measure_name].nrows);
            var curr_row = glob_ii-curr_col*nrows;
            ...
            legend.append("text")
                  ...
                  .style("font-size", "small");
            legend.append("text")
                  .attr("x", col_width-48 + col_width*curr_col + col_sep*(curr_col))
                  .attr("y", (curr_row+1)*15)
                  .attr("dy", ".35em")
    ...
    do_one_plot("#plot_fb" , "fb" , "PASCALContext", "test_new")
    do_one_plot("#plot_fop", "fop", "PASCALContext", "test_new")
